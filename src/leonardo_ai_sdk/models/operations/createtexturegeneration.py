...
class TextureGenerationJobOutputTypedDict(TypedDict):
    ...
    r"""API Credits Cost for Texture Generation. Available for Production API Users. Note: it will be deprecated. Please use the cost instead."""
...
class CreateTextureGenerationResponseBody(BaseModel):
    ...
    @model_serializer(mode="wrap")
    def serialize_model(self, handler):
        optional_fields = set(["textureGenerationJob"])
        nullable_fields = set(["textureGenerationJob"])
        serialized = handler(self)
        m = {}
